...
const carInfoFinder = (carArray, carNum) => {
    carArray.forEach((item) => {
        (item.id === carNum ? console.log(`Car ${carNum} is a ${item.car_year} ${item.car_make} ${item.car_model}`):null)
    })
}

carInfoFinder(inventory, 33);
...
const lastCar = (carArray) => console.log(`The last car in the inventory is a ${carArray[carArray.length-1].car_make} ${carArray[carArray.length-1].car_model}`)
...
const carYearFinder = (carArray, newArray) => {
    carArray.forEach((item) => newArray.push(item.car_year))
}

carYearFinder(inventory, carYears);
...
const findOldCars = (carArray, carYear, newArray) => {
    carArray.forEach((item) => {
        (item.car_year < carYear ? newArray.push(item):null)
    })
}

findOldCars(inventory, 2000, oldCars);
...
const findCars = (carArray, carMake1, carMake2, newArray) => {
    carArray.forEach((item) => {
        ((item.car_make === carMake1 || item.car_make === carMake2) ? newArray.push(item):null)
    })
}

findCars(inventory, 'BMW', 'Audi', BMWAndAudi)
